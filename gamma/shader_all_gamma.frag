...
void main () {
    ...
    vec3 color = gammaCorrection( vec3(texture2D(tex, uv).g), gamma );

   gl_FragColor = colormap( vec3 (gammaCorrection( vec3(texture2D(tex, uv).g), gamma )).g, A, B, C );
//   gl_FragColor = colormap( texture2D(tex, uv).g, A, B, C );
