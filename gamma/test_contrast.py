...
with open("shader_all_gamma.frag", "r") as f:
    fragment_shader_src = '"\n' + f.read() + '"'
...
###CONTRAST
gamma = GObject.Value(GObject.TYPE_FLOAT)
gamma.set_float(1.0)
####!!!CONTRAST
scale.set_float(1.0)
A.set_int(3)
...
uniforms.set_value("gamma", gamma)
...
def sigamma_handler(signum, frame):
    global gamma_val
    # Adjust the zoom level here, for example:
    gamma_val = 3.0 if gamma_val > 3.0 else gamma_val=gamma_val + 0.3
    print("Received SIGWINCH, zoom is: ", zoomindex + 1 , "x")
    set_zoom_level( zoomlevel[zoomindex] )  # 2x zoom

# Register the signal handler
signal.signal(signal.SIGPOLL, sigzoom_handler)
signal.signal(signal.SIGPWR, sigpalette_handler)
signal.signal(signal.SIG, sigamma_handler)
